...
majorlist = []
action = []
# Creates a list 'action' to contain all those who have had action taken against them
for id, info in student_info.items():
    ...
    if DA != '':
        action.append((id, last_name, first_name, date))
# the disciplinary action column is left blank for those who have not received action, thus the if statement
# ensures that if there is any value in that column, it is recorded and added to the list
def sort_key(x):
    return x[3]

action.sort(key=sort_key)

with open('FinalProjectDisciplinedStudents.csv', 'w', newline='') as newfile:
