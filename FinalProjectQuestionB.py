...
majorlist = []
permajor = []
# Creates a list 'permajor' to hold nested lists and majorlist to contain the different majors with no duplicates
for id, info in student_info.items():
    ...
    if major not in majorlist:
        majorlist.append(major)
    for item in majorlist:
        permajor.append([])
    if major == majorlist[0]:
        permajor[0].append((id, last_name, first_name, date, DA))
    elif major == majorlist[1]:
        permajor[1].append((id, last_name, first_name, date, DA))
    elif major == majorlist[2]:
        permajor[2].append((id, last_name, first_name, date, DA))
    elif major == majorlist[3]:
        permajor[-1].append((id, last_name, first_name, date, DA))
# ^^^creates lists for each different major
for item in majorlist:
    filename = item.strip()
    with open(f'{filename}Students.csv', 'w', newline='') as newfile:
        writer = csv.writer(newfile)
        for item in permajor:
            writer.writerow(item)
# ^^^ writes the csv files for each major
